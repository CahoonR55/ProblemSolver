...
public class PuzzleState implements State {
    ...
    */
    ...
    public static int tilesOutOfPlace(State current, State goal) {
        ...
        for(int i = 0; i < 3; ++i){
            for(int j = 0; j < 3; ++j){
                if(curState.getTiles()[i][j] != goalState.getTiles()[i][j] && curState.getTiles()[i][j] != 0) {
                    ++count;
                };
            }
        }
    ...
    public static int sumManhattan(State current, State goal) {
        ...
        for(int i = 0; i < 3; ++i){
            for(int j = 0; j < 3; ++j){
                if(curState.getTiles()[i][j] != goalState.getTiles()[i][j] && curState.getTiles()[i][j] != 0) {
                    xSum += Math.abs(goalState.getLocation(curState.getTiles()[i][j]).getColumn() - j);
                    ySum += Math.abs(goalState.getLocation(curState.getTiles()[i][j]).getRow() - i);
                }
            }
        }
